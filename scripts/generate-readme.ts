...
class ReadmeGenerator {
  ...
  generateFromWorkflow(workflowPath: string): string {
    try {
      ...
      if (!fs.existsSync(this.templatePath)) {
        throw new Error(
          "README template not found at templates/workflow-readme.md",
        );
      }
      ...
      let readmeContent = template
        ...
        .replace(
          /{Processing Steps}/g,
          this.generateProcessingSteps(processingNodes),
        )
        .replace(
          /{Dependencies}/g,
          this.generateDependencies(metadata.requirements),
        )
      ...
      throw new Error(
        `Failed to generate README for ${workflowPath}: ${errorMessage}`,
      );
    }
  }
  ...
  private generateDependencies(requirements?: WorkflowRequirements): string {
    ...
    if (requirements.environment_variables?.length) {
      deps.push(
        `**Environment Variables**: ${requirements.environment_variables.join(
          ", ",
        )}`,
      );
    }
    ...
    for (const workflow of workflows) {
      ...
      try {
        ...
        const errorMessage =
          error instanceof Error ? error.message : "Unknown error";
        console.log(
          `  ❌ Failed to generate README for ${workflow}: ${errorMessage}`,
        );
        failed++;
      }
    }
