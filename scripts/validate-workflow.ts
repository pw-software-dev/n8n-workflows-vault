...
class WorkflowValidator {
  ...
  validateWorkflowFolder(folderPath: string): ValidationResult {
      ...
      try {
        ...
        if (!workflow.connections) {
          results.warnings.push(
            "workflow.json should contain connections object",
          );
        }
        ...
        if (
          this.workflowSchema &&
          !this.ajv.validate(this.workflowSchema, workflow)
        ) {
          results.errors.push(
            `Workflow schema validation failed: ${this.ajv.errorsText()}`,
          );
          results.valid = false;
        }
      } catch (error) {
    ...
    if (fs.existsSync(metadataPath)) {
      try {
        ...
        const requiredFields: (keyof WorkflowMetadata)[] = [
          "name",
          "description",
          "version",
        ];
        const missingMetaFields = requiredFields.filter(
          (field) => !metadata[field],
        );

        if (missingMetaFields.length > 0) {
          results.errors.push(
            `Missing required metadata fields: ${missingMetaFields.join(", ")}`,
          );
          results.valid = false;
        }
        ...
        if (
          this.metadataSchema &&
          !this.ajv.validate(this.metadataSchema, metadata)
        ) {
          results.errors.push(
    ...
    if (fs.existsSync(readmePath)) {
      ...
      if (missingSections.length > 0) {
        results.warnings.push(
          `README.md missing recommended sections: ${missingSections.join(
            ", ",
          )}`,
        );
      }
  ...
  checkConsistency(): void {
    ...
    this.scanAllWorkflows(
      (workflowPath: string, metadata: WorkflowMetadata) => {
        if (metadata.name) {
  ...
  private scanAllWorkflows(
    callback: (workflowPath: string, metadata: WorkflowMetadata) => void,
  ): void {
    const workflowsDir = "./workflows";
    const workflows = fs
      .readdirSync(workflowsDir, { withFileTypes: true })
